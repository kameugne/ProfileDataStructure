...
public class NewEdgeFinder {
    ...
    public NewEdgeFinder(Task[] tasks, int C)
    {
        this.C = C;
        this.tasks = tasks;
        this.tl = new Profile();
    ...
    public int[] Filter()
    {
        ...
        if(NewEdgeFinder_Detection())
            results = NewEdgeFinder_Pruning();
        return results;
    }
    ...
    private boolean NewEdgeFinder_Detection()
    {
        InitializeIncrements(tasks_indices_lct.length - 1);
        ...
        for(int i = 0; i < tasks_indices_lct.length -1; i++)
        {
            ...
            {
                tasks[tasks_indices_lct[i+1]].inLambda = true;
                i++;
            }
            if(i == tasks.length-1)
                return true;
    ...
    private int[] NewEdgeFinder_Pruning()
    {
        ...
        for(int i=0; i<tasks.length; i++){
            if(Prec[i] != -1)
            {
                estPrime[i] =  Math.max(estPrime[i], est[i]);
                /*if(type[i] == 0){
                    estPrime[i] =  Math.max(estPrime[i], est[i]);
                }else{
                    ...
                    if (maxOv > 0) {
                        int estt = ComputeBound(tasks[i].height(), maxOv);
                        if (estt > tasks[i].earliestStartingTime()) {
                            estPrime[i] = Math.max(estPrime[i], estt);
                        }
                    }
                }*/
            }
    ...
    private void DetectePrecedences(int j, int min, int max){
        for(int i = min; i <= max; i++){
            if(!tasks[tasks_indices_h[i]].inLambda && tasks[tasks_indices_h[i]].est_to_timepoint.contact != null &&
                ...
                if(tasks[tasks_indices_h[i]].earliestCompletionTime() < tasks[tasks_indices_lct[j]].latestCompletionTime()){
                    ...
                    if(overlap2 - overlap1 > slackUnder2 - slackUnder1 + slackOver2 - slackOver1){
                        ...
                        est[tasks_indices_h[i]] = estt;
                        if(slackUnder2 == slackUnder1 && slackOver2 == slackOver1){
                            type[tasks_indices_h[i]] = 0;
                        }
                    }
                }else{
                    Timepoint t = tasks[tasks_indices_h[i]].est_to_timepoint.contact;
